...
class ElementsAlertsArea(BasePage):
    ...
    def validate_page_load(self):
        assert (self.is_displayed(self.alert_button_loc, timeout=15)) and \
    ...
    def validate_timer_alerts_button_text(self):
        assert self.is_alert_present(timeout=15) and \
    ...
    def validate_confirm_button_close_accept(self):
        assert (not self.is_alert_present(1)) and ('You selected Ok' in self.driver.page_source)

    def validate_confirm_button_close_dismiss(self):
        assert (not self.is_alert_present(1)) and ('You selected Cancel' in self.driver.page_source)
    ...
    def validate_prompt_button_close_accept(self):
        assert (not self.is_alert_present(1)) and ('You entered Billy' in self.driver.page_source)
