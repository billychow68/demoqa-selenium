...
class ElementsTextboxArea(BasePage):
    ...
    def validate_page_load(self):
        if self.is_displayed(self.full_name_loc, 15) and \
           self.is_displayed(self.full_name_label_loc, 10) and \
           self.is_displayed(self.email_loc, 15) and \
           self.is_displayed(self.email_label_loc, 15) and \
           self.is_displayed(self.current_address_loc, 15) and \
           self.is_displayed(self.current_address_label_loc, 15) and \
           self.is_displayed(self.elememts_permanentaddress_loc, 15) and \
           self.is_displayed(self.permanent_address_label_loc, 15) and \
           self.is_displayed(self.submit_button_loc, 15):
            return True
        else:
            return False

    def textbox_submit_form(self, name, email, cur_addr, perm_addr):
        if self.is_displayed(self.full_name_loc, 15):
            self.send_keys(self.full_name_loc, name)
        if self.is_displayed(self.email_loc, 15):
        ...
        if self.is_displayed(self.submit_button_loc, 15):
