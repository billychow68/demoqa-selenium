...
class ElementsUploadDownloadArea(BasePage):
    ...
    def validate_page_load(self):
        return self.is_displayed(self.download_button_loc, timeout=15) and \
    ...
    def validate_download(self, json_loader):
        ...
        config = json_loader(path)
        full_path = os.path.join(config["path"], config["file"])
        if self.is_file_exists(full_path, timeout=15):
            self._delete_sample_file(json_loader)
            return True
        return False
    ...
    def validate_upload(self):
        return self.is_text_displayed(self.upload_file_path_loc, "C:\\fakepath\sampleFile.jpeg", timeout=15)
